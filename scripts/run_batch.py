...
CONFIGS = ["CASH", "SPB", "Hybrid", "HybridNoInference"]
...
def validate(record):
    ...
    if bound_is_number(low) and bound_is_number(high) and int(low) > int(high):
        problems.append(f"lower bound {low} exceeds upper bound {high}")
    if record.get("cash_proved_optimal"):
        if not bound_is_number(high):
            problems.append("claims optimality without an upper bound")
        elif not bound_is_number(low) or int(low) != int(high):
            problems.append(f"claims optimality but bounds differ: {low} vs {high}")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in futures:
            try:
                ...
                log(f"FAIL worker raised: {error!r}")
                stopper.stop(str(error))

    summarise(args, statuses, meta)

    failed = [s for s in statuses if s["problems"]]
    if stopper.stopped:
        log(f"BATCH STOPPED EARLY: {stopper.reason}")
